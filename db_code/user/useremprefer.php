...
<div class="layout">
  ...
  <div class="flexbox">
    <div class="item">
    <form action = "useremprefer.php" method="post">
      <h4>==분류 조회==</h4>
...
if ( !$con = oci_connect("DBA2022G6","test1234",$db,"AL32UTF8")) exit();
   ...
   $sql= "SELECT emp.empID, emp.empName, firestation.areaName, firestation.fsName, emp.dept, emp.Estatus FROM emp inner JOIN firestation ON emp.fsID = firestation.fsID WHERE emp.dept='$dept' AND firestation.areaName='$areaName' AND firestation.fsName='$fsName'";
   ...
   echo "<TABLE border=1>";
   echo "<TR>";
   echo "<TH>직원 ID</TH><TH>직원 이름</TH><TH>관할 지역</TH><TH>안전 센터</TH><TH>부서</TH><TH>출동현황</TH>";
   echo "</TR>";
...
?>
  ...
  <form method="get" name="form" action="useremprefer.php">
<h4>==검색 조회==</h4>
    직원번호로 조회&ensp;&ensp;<input type="text" name="empID" placeholder="번호만 입력">
    <input type="submit" value="조회" onclick="send()"><br>
    이름으로 조회&emsp;&emsp;<input type="text" name="empName" placeholder="영어로 입력">
...
$sql= "SELECT emp.empID, emp.empName, firestation.areaName, firestation.fsName, emp.dept, emp.Estatus FROM emp inner JOIN firestation ON emp.fsID = firestation.fsID WHERE emp.empID='$empID' OR emp.empName='$empName'";
$stat = oci_parse($con, $sql);
$ret = oci_execute($stat);
if(!$ret) {  // 오류라면 메시지 출력하고 종료
echo "SQL문 오류"."<br>";
