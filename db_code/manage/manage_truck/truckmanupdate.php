...
(DESCRIPTION =
   ...
   $sql ="SELECT * FROM fireT WHERE truckID='$truckID'";
   ...
   while(($row = oci_fetch_array($stat, OCI_BOTH)) != false) {
    echo "<FORM METHOD=\"post\"  ACTION=\"truckmanupdateac.php\">\n";
    echo "       차량ID :&nbsp;<input type=\"text\" name = \"truckID\" VALUE= $row[0] READONLY ><br><br>\n";
    echo "       안전센터 ID :&nbsp;<input type=\"text\" name = \"fsID\" VALUE=$row[1]><br><br>\n";
    echo "       제조년도:&nbsp;<input type=\"text\" name = \"Made\" VALUE=$row[2]><br><br>\n";
    echo "       관리기간 :&nbsp;<input type=\"text\" name = \"Cyear\" VALUE=$row[3]><br><br>\n";
    echo "       출동현황 :&nbsp;<input type=\"text\" name = \"Tstatus\" VALUE=$row[4]><br><br>\n";
    echo "           <BR>\n";
