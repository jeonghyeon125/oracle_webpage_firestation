...
<body>
  <header><h1>소방차량 DATA 조회</h1></header>
  <div>
  <form action = "firetruckrefer.php" method="post">
  ...
  <div style="white-space:nowrap; overflow:auto;  width:80%; height:100%; margin:0 auto;">
    ...
    if(!$ret) { echo "SQL문 오류"."<br>"; exit();}
    echo "<TABLE border=1>";
    echo "<TR>";
    echo "<TH>차량변호</TH><TH>관할자역</TH><TH>안전센터</TH><TH>제조일</TH><TH>관리기간</TH><TH>출동현황</TH>";
  ...
  <div style="white-space:nowrap; overflow:auto;  width:80%; height:400px; margin:0 auto;">
    ...
    $sql= "SELECT fireT.truckID, firestation.areaName, firestation.fsName, fireT.Made, fireT.Cyear, fireT.Tstatus FROM fireT inner JOIN firestation ON fireT.fsID = firestation.fsID";
    $stat = oci_parse($con, $sql);
    $ret = oci_execute($stat);
