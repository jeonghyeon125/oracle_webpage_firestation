  ...
  <div class="idcheck">
    ...
    if(isset($_SESSION['adminCode']) && $_SESSION['userID']){
        ...
        if(isset($_SESSION['userID'])){
          ...
          echo "<script>alert('로그인이 필요합니다.\\n직원 로그인 후 관리자 로그인을 진행해주세>요')</script>";
...
<body>
  <header><h1>동아리 DATA 조회</h1></header>
  <div>

            <form action = "clubrefer.php" method="post">
                        ...
                        <input type="text" name="clubName" placeholder="소속 동아리(영어로 입>력)">
...
$sql = "SELECT emp.empID, emp.empName, club.clubName, firestation.areaName, firestation.fsName, emp.dept
FROM emp
...
WHERE firestation.areaName = '$areaName' and firestation.fsName = '$fsName' and emp.dept = '$dept' and club.clubName = '$clubName'";
...
echo "<TABLE border=1>";
echo "<TR>";
echo "<TH>직원 번호</TH><TH>직원 이름</TH><TH>소속 동아리</TH><TH>관할구역</TH><TH>안전센터</TH><TH>부서</TH>";
...
(DESCRIPTION =
   ...
   if(!$ret) {  // 오류라면 메시지 출력하고 종료
        ...
        echo "<TABLE border=1>";
        echo "<TR>";
        echo "<TH>직원 번호</TH><TH>직원 이름</TH><TH>소속 동아리</TH><TH>관할구역</TH><TH>안>전센터</TH><TH>부서</TH>";
        echo "</TR>";
